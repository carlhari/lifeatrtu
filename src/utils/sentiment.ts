import { tlGood, tlWords } from "@/utils/profanity";
const vader = require("crowd-sentiment");

export const sentimentAnalyzer = async (text: string, origText: string) => {
  const translated =
    await vader.SentimentIntensityAnalyzer.polarity_scores(text);
  ...
  //-----------------------------------------------------------
  const translatedGBoost = tlGood.some((word) =>
    text.toLowerCase().includes(word),
  );

  const origGBoost = tlGood.some((word) =>
    origText.toLowerCase().includes(word),
  );
  ...
  if (origGBoost || translatedGBoost) {
    intensity += 0.3;
  }
